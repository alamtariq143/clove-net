...
for site in wonderware_sites:
    ...
    in_sap_mseg = spark.read.option("header", True).csv(
        '{}/{}'.format(source, 'RFM_MES_MSEG.csv'))
    ...
    ds_availability = ds_availability.withColumn(
        'target_prod_rate',
         when(col('target_prod_rate')=='-', lit(None).cast('string'))
         .otherwise(col('target_prod_rate'))
    )
    ...
    ds_availability = ds_availability.withColumn(
        'target_prod_rate',
         when(col('target_prod_rate')=='empty', lit(None).cast('string'))
         .otherwise(col('target_prod_rate'))
    )
...
mseg = (
    mseg
    .withColumn('entry_ts', concat_ws(' ', col('cpudt_mkpf').cast('string'), col('cputm_mkpf')))
    .withColumn('menge_kg', 
...
sap_mseg.write.mode("overwrite").parquet(
    '{}/sap_mseg/'.format(destination))"""
    
    
availability.write.mode("overwrite").parquet(
    '{}/availability_test/'.format(destination))

mseg.write.mode("overwrite").parquet(
    '{}/mseg_test/'.format(destination))

mseg_q1.write.mode("overwrite").parquet(
    '{}/mseg_q1_test/'.format(destination))

oee.write.mode("overwrite").parquet(
    '{}/oee_test/'.format(destination))

opn.write.mode("overwrite").parquet(
    '{}/opn_test/'.format(destination))

po.write.mode("overwrite").parquet(
    '{}/po_test/'.format(destination))
    
rt_ftr.write.mode("overwrite").parquet(
    '{}/rt_ftr_test/'.format(destination))
    
ve.write.mode("overwrite").parquet(
    '{}/ve_test/'.format(destination))
 
q1_summary.write.mode("overwrite").parquet(
    '{}/q1_summary_test/'.format(destination))

po_summary.write.mode("overwrite").parquet(
    '{}/po_summary_test/'.format(destination))

step_summary.write.mode("overwrite").parquet(
    '{}/step_summary_test/'.format(destination))

keys.write.mode("overwrite").parquet(
    '{}/keys_test/'.format(destination))

sap_mseg.write.mode("overwrite").parquet(
    '{}/sap_mseg_test/'.format(destination))
